...
`復帰` は区分として廃止し、期限付き移籍先から戻る動きは `期限付移籍out終了` として扱います。`加入` は `移籍in` と意味が重複するため区分として廃止します。`退団` は `移籍out`、`契約満了`、`引退` などと重複するため区分として廃止します。`新卒加入` はオフシーズン動向では扱わないため、区分として設けません。`レンタル中` はイベント区分ではなく状態なので、`区分` には使いません。レンタル中であることや期限は `その他備考` に記載します。移籍系、期限付き移籍系では、スマホ表示シートやPDFの印刷範囲 `A:F` で見えるように、移籍元または移籍先を `移籍先元` だけでなく `その他備考` にも必ず記載します。

`移籍in`、`期限付移籍in`、`期限付移籍out終了` が公式サイトで確認できた場合は、スマホ表示シートの母集団を保つため `official_events.csv` だけでなく `players.csv` も追加・更新します。背番号が公式サイトで確認できない場合、背番号欄は空欄にし、`未確認` などの文字列は入れません。
...
- CSVはこのリポジトリの正本データです。
- Codexは公式サイトだけを確認し、契約更新、移籍in、移籍out、期限付移籍in、期限付移籍out、期限付移籍in延長、期限付移籍out延長、期限付移籍in終了、期限付移籍out終了、契約満了、引退、離脱などの公式発表が確認できた場合だけCSVへ反映します。
