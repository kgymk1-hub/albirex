...
- `期限付移籍out終了`: アルビから他クラブへ期限付き移籍していた選手の期限付き移籍期間が終了する動き。加入・復帰寄りとして扱い、色も加入系にします。ただし、復帰が別途公式発表された場合は `復帰` イベントを別に記録し、期限付き移籍終了と復帰を同一視しません。
...
- CSVはこのリポジトリの正本データです。
- Codexは公式サイトだけを確認し、契約更新、復帰、移籍in、移籍out、期限付移籍in、期限付移籍out、期限付移籍in延長、期限付移籍out延長、期限付移籍in終了、期限付移籍out終了、契約満了、引退などの公式発表が確認できた場合だけCSVへ反映します。
