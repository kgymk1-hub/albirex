...
- `移籍in`: 他クラブから完全移籍で加入する動き。旧表記の `完全移籍加入`、`移籍加入` はこの区分に整理します。
- `移籍out`: 他クラブへ完全移籍する動き。旧表記の `完全移籍` はこの区分に整理します。
- `期限付移籍in`: 他クラブから期限付き移籍で加入する動き。旧表記の `期限付き移籍加入` はこの区分に整理します。
- `期限付移籍out`: 他クラブへ期限付き移籍する動き。旧表記の `期限付き移籍` はこの区分に整理します。
- `期限付移籍in延長`: 他クラブからの期限付き移籍加入期間を延長する動き。
- `期限付移籍out延長`: 他クラブへの期限付き移籍期間を延長する動き。

`レンタル中` はイベント区分ではなく状態なので、`区分` には使いません。レンタル中であることや期限は `その他備考` に記載します。移籍系、期限付き移籍系、復帰では、スマホ表示シートやPDFの印刷範囲 `A:F` で見えるように、移籍元または移籍先を `移籍先元` だけでなく `その他備考` にも必ず記載します。

`加入`、`復帰`、`移籍in`、`期限付移籍in` が公式サイトで確認できた場合は、スマホ表示シートの母集団を保つため `official_events.csv` だけでなく `players.csv` も追加・更新します。背番号が公式サイトで確認できない場合、背番号欄は空欄にし、`未確認` などの文字列は入れません。
...
- CSVはこのリポジトリの正本データです。
- Codexは公式サイトだけを確認し、契約更新、加入、復帰、移籍in、移籍out、期限付移籍in、期限付移籍out、期限付移籍in延長、期限付移籍out延長、退団、契約満了、引退などの公式発表が確認できた場合だけCSVへ反映します。
