...
背番号は `data/*/players.csv` を正本とします。スマホ表示シートのA列には `players.csv` の背番号が反映されます。公式サイトで背番号変更が確認できた場合は、`players.csv` の背番号を更新します。公式サイトで背番号が確認できない選手は、背番号を空欄のままにします。推測や非公式情報による背番号補完は行いません。背番号不明時は `未確認` と入力せず、空欄にします。`players.csv` はヘッダー行を除き、背番号が数値で入っている選手を数値昇順に並べ、背番号空欄の選手は背番号あり選手の後ろに置きます。背番号空欄の選手同士は既存の相対順をできるだけ維持します。
...
- `期限付移籍out終了`: アルビから他クラブへ期限付き移籍していた選手の期限付き移籍期間が終了する動き。加入・復帰寄りとして扱い、色も加入系にします。公式発表本文に「復帰」とあっても、CSVの `区分` と `動向` では `復帰` を使わず `期限付移籍out終了` に統一します。
- `期限付移籍in終了`: 他クラブからアルビへ期限付き移籍加入していた選手の期限付き移籍期間が終了する動き。退団・離脱系として扱い、色も退団系にします。

`復帰` は区分として廃止し、期限付き移籍先から戻る動きは `期限付移籍out終了` として扱います。`新加入` は公式発表が新加入内定の場合に使用します。`退団` は `移籍out`、`契約満了`、`引退` などと重複するため区分として廃止します。`新卒加入` はオフシーズン動向では扱わないため、区分として設けません。`レンタル中` はイベント区分ではなく状態なので、`区分` には使いません。レンタル中であることや期限は `その他備考` に記載します。移籍系、期限付き移籍系では、スマホ表示シートやPDFの印刷範囲 `A:F` で見えるように、移籍元または移籍先を `移籍先元` だけでなく `その他備考` にも必ず記載します。

`移籍in`、`新加入`、`期限付移籍in`、`期限付移籍out終了` が公式サイトで確認できた場合は、スマホ表示シートの母集団を保つため `official_events.csv` だけでなく `players.csv` も追加・更新します。背番号が公式サイトで確認できない場合、背番号欄は空欄にし、`未確認` などの文字列は入れません。
...
- CSVはこのリポジトリの正本データです。
- Codexは公式サイトだけを確認し、契約更新、移籍in、新加入、移籍out、期限付移籍in、期限付移籍out、期限付移籍in延長、期限付移籍out延長、期限付移籍in終了、期限付移籍out終了、契約満了、引退、離脱などの公式発表が確認できた場合だけCSVへ反映します。
