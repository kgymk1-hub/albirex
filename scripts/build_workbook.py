...
This script is intended to run in GitHub Actions only. Codex should edit this
text file, but should not execute it in this environment because running it
creates .xlsx files.
"""
...
def event_sort_key(index: int, event: dict[str, str]) -> tuple[date, int]:
    """Return sort key for one player's events.

    Newer 発表日 wins. If 発表日 is blank/invalid or the date is the same, the
    later row in the CSV wins. Dates should be normalized to YYYY-MM-DD where
    possible in the CSV.
    """
    raw_date = (event.get("発表日") or "").strip()
    try:
...
def latest_events_by_player(events: list[dict[str, str]]) -> dict[str, dict[str, str]]:
    latest: dict[str, tuple[tuple[date, int], dict[str, str]]] = {}
    for index, event in enumerate(events):
        player_name = event.get("選手名", "")
        if not player_name:
            continue
        key = event_sort_key(index, event)
        if player_name not in latest or key >= latest[player_name][0]:
            latest[player_name] = (key, event)
    return {player_name: event for player_name, (_key, event) in latest.items()}
...
def build_mobile_rows(players: list[dict[str, str]], events: list[dict[str, str]]) -> list[list[str]]:
    events_by_name = latest_events_by_player(events)
    rows = []
    for player in players:
        event = events_by_name.get(player.get("選手名", ""), {})
        ...
        rows.append([
            ...
            movement,
            event.get("発表日", ""),
            event.get("その他備考", "") or player.get("その他備考", ""),
            category,
            event.get("移籍先元", ""),
            status,
            event.get("出典URL", ""),
...
def setup_mobile_sheet(ws, rows: list[list[str]]) -> None:
    ...
    set_widths(ws, {"A": 7, "B": 9, "C": 18, "D": 16, "E": 12, "F": 28, "G": 14, "H": 18, "I": 14, "J": 12})
...
def build_workbook(team_key: str, output_name: str) -> None:
    ...
    setup_mobile_sheet(wb.create_sheet("スマホ表示"), build_mobile_rows(players, events))
    setup_simple_sheet(wb.create_sheet("選手マスタ"), PLAYER_HEADERS, ([row.get(header, "") for header in PLAYER_HEADERS] for row in players), {"A": 8, "B": 12, "C": 22, "D": 40})
    setup_simple_sheet(wb.create_sheet("オフ動向_入力"), EVENT_HEADERS, ([row.get(header, "") for header in EVENT_HEADERS] for row in events), {"A": 22, "B": 18, "C": 12, "D": 18, "E": 24, "F": 14, "G": 48, "H": 40})
    setup_simple_sheet(wb.create_sheet("区分マスタ"), ["区分", "分類", "色"], CATEGORY_MASTER, {"A": 20, "B": 14, "C": 12})
    setup_simple_sheet(wb.create_sheet("確認ログ"), LOG_HEADERS, [], {"A": 14, "B": 24, "C": 36, "D": 18, "E": 30})
    memo_rows = [
        ["このExcelファイルは直接編集せず、dataフォルダ内のCSVを更新してGitHub Actionsで再生成する。"],
        ["スマホ表示シートの印刷範囲は A:F とする。"],
        ["G列以降は管理用情報であり、印刷対象外とする。"],
        ["PDFはGitHub Actions上で、スマホ表示シートの印刷設定を使って生成する。"],
    ]
    setup_simple_sheet(wb.create_sheet("メモ"), ["運用上の注意点"], memo_rows, {"A": 100})
