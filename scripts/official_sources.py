...
EVENT_KEYWORDS = {
    "期限付き移籍延長": ["期限付き移籍期間延長", "期限付き移籍延長"],
    "期限付き移籍加入": ["期限付き移籍加入", "育成型期限付き移籍加入"],
    "契約満了": ["契約満了"],
    "契約更新": ["契約更新"],
    "完全移籍": ["完全移籍"],
    "レンタル中": ["期限付き移籍のお知らせ", "期限付き移籍移行"],
    "復帰": ["復帰"],
    "退団": ["退団"],
    "引退": ["引退"],
    "加入": ["加入", "新加入", "内定"],
...
def _build_event(title: str, article_text: str, category: str, url: str, known_names: set[str]) -> OfficialEvent | None:
    ...
    movement = title
    status = "在籍予定" if category in {"契約更新", "加入", "復帰", "期限付き移籍加入", "期限付き移籍延長"} else category
    return OfficialEvent(player_name, movement, announced_on, category, "", status, url, "公式サイト確認")
